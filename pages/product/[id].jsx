...
const Product = ({ pizza }) => {
  ...
  const [mainImg, setMainImg] = useState(pizza.imgs?.[0] || pizza.img); // main displayed image
      ...
      <div className={styles.left}>
        ...
        <div className={styles.thumbnails}>
          {(pizza.imgs || [pizza.img]).map((img, index) => (
            <div key={index} className={styles.thumbnail} onClick={() => setMainImg(img)}>
              <Image src={img} layout="fill" objectFit="cover" alt={`thumbnail-${index}`} />
            </div>
          ))}
        </div>
      </div>
...
export const getServerSideProps = async ({ params }) => {
  ...
  try {
    ...
    console.error(err);
    return {
      props: { pizza: null }, // fallback if product not found
    };
  }
};